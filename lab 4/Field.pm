...
sub view {
	...
	my @view;
	foreach (@array){
		my @temp = map {defined $_ ? $_->owner() == $player ? $_->owner() : 0 : 0} @$_;
		push @view, \@temp;
	}
	return @view;
}
...
sub shoot {
	...
	if (not defined $array[$piece_x][$piece_y]){
		return "Invalid move";
...
sub ended {
	if (!grep {defined $_} ($p2_s1,$p2_s2,$p2_s3,$p2_s4,$p2_s5)){
		return "Player 1 has won the game";
	} elsif (!grep {defined $_} ($p1_s1,$p1_s2,$p1_s3,$p1_s4,$p1_s5)){
